...
export default function ClientSettingsPage() {
  ...
  const handleAddSheet = async (e) => {
    ...
    try {
      ...
      setSheetSuccess("Sheet connected successfully!");
      ...
      <section className="bg-[#111] border border-[#1e1e1e] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[#444] text-xs mt-1">{sheets.length} of {maxSheets} sheets connected</p>
          </div>
          {googleConnected ? (
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5 text-xs text-[#00c853] bg-[#00c853]/10 border border-[#00c853]/20 px-2.5 py-1.5 rounded-lg font-medium">
                ✓ Google connected
              </span>
              <button onClick={openGoogleAuth} className="text-xs text-[#333] hover:text-[#555] transition">Reconnect</button>
            </div>
          ) : (
            <button onClick={openGoogleAuth} disabled={connectingGoogle}
              ...
              {connectingGoogle ? "Waiting…" : "Connect Google"}
            </button>
          )}
        </div>
        ...
        {/* Success message */}
        {sheetSuccess && (
          <div className="bg-[#00c853]/10 border border-[#00c853]/20 text-[#00c853] text-xs px-3 py-2 rounded-lg mb-3">
            ✓ {sheetSuccess}
          </div>
        )}
        ...
        {sheets.length === 0 ? (
          <div className="text-center py-6 text-[#333] text-sm border border-dashed border-[#1e1e1e] rounded-xl mb-4">
            No sheets connected yet
          </div>
        ) : (
          <div className="space-y-2 mb-4">
            {sheets.map((s, i) => (
              <div key={s.id} className="flex items-center gap-3 bg-[#0a0a0a] border border-[#1a1a1a] rounded-xl px-4 py-3">
                <div className="w-7 h-7 rounded-lg bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center text-[#444] text-xs font-bold shrink-0">
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium truncate flex items-center gap-2">
                    {s.spreadsheet_name || "Untitled Sheet"}
                    {s.is_primary && <span className="text-[10px] text-[#00c853] border border-[#00c853]/30 px-1.5 py-0.5 rounded-full shrink-0">Primary</span>}
                  </p>
                  <p className="text-xs text-[#444] font-mono mt-0.5 truncate">#{s.spreadsheet_id?.slice(0, 20)}…</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {s.tab_count > 0 && <span className="text-[10px] text-[#444] border border-[#1e1e1e] px-1.5 py-0.5 rounded">{s.tab_count} tabs</span>}
                  ...
                  <button onClick={() => handleRemoveSheet(s.id)} className="text-xs text-[#333] hover:text-red-400 transition">✕</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {sheets.length < maxSheets && (
          <>
            {!showAddForm ? (
              <button
                onClick={() => { setShowAddForm(true); setSheetErr(""); }}
                className="w-full flex items-center justify-center gap-2 border border-dashed border-[#2a2a2a] hover:border-[#00c853]/40 hover:text-[#00c853] text-[#444] text-sm py-3 rounded-xl transition"
              >
                + Add another sheet
              </button>
            ) : (
              <form onSubmit={handleAddSheet} className="border border-[#2a2a2a] rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs text-[#555] font-semibold uppercase tracking-wider">New Sheet</p>
                  <button type="button" onClick={() => { setShowAddForm(false); setSheetErr(""); setSheetInput(""); setSheetName(""); }}
                    className="text-xs text-[#333] hover:text-white transition">✕ Cancel</button>
                </div>
                <div>
                  <label className="block text-xs text-[#555] mb-1.5">Sheet URL</label>
                  <input
                    value={sheetInput}
                    onChange={(e) => { setSheetInput(e.target.value); setSheetErr(""); }}
                    className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#333] focus:outline-none focus:border-[#00c853]/50 transition"
                ...
                <div>
                  <label className="block text-xs text-[#555] mb-1.5">Name <span className="text-[#333]">(optional — auto-detected if blank)</span></label>
                  <input
                    value={sheetName}
                    onChange={(e) => setSheetName(e.target.value)}
                    className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#333] focus:outline-none focus:border-[#00c853]/50 transition"
                    placeholder="e.g. Sales Q1, Inventory, Customer Data…"
                  />
                </div>
                {sheetErr && <p className="text-red-400 text-xs">{sheetErr}</p>}
                <button type="submit" disabled={addingSheet || !sheetInput.trim()}
                  className="w-full bg-[#00c853] hover:bg-[#00b248] text-black font-bold py-2.5 rounded-xl text-sm transition disabled:opacity-50">
                  {addingSheet ? "Connecting…" : "Connect this sheet →"}
                </button>
              </form>
            )}
          </>
        ...
        {sheets.length >= maxSheets && (
          <p className="text-xs text-[#444] text-center mt-2">
            Maximum sheets reached ({maxSheets}). Contact support to upgrade your plan.
          </p>
        )}
      </section>
